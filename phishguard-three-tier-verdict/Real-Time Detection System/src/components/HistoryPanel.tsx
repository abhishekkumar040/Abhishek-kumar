                ...
                <tr key={idx} className="hover:bg-slate-800/50 transition-colors">
                  <td className="px-4 py-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                      result.trustLevel === 'phishing' ? 'bg-red-500/20' :
                      result.trustLevel === 'verified_legitimate' ? 'bg-emerald-500/20' :
                      'bg-slate-500/20'
                    }`} title={
                      result.trustLevel === 'phishing' ? 'Phishing detected' :
                      result.trustLevel === 'verified_legitimate' ? 'Verified legitimate' :
                      'No red flags, but domain not recognized'
                    }>
                      {result.trustLevel === 'phishing' ? (
                        <svg className="w-4 h-4 text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                      ) : result.trustLevel === 'verified_legitimate' ? (
