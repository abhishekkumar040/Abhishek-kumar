...
const verdictConfig = {
  ...
  // FIX #3e: collapsed to the simple green/red scheme requested - "no red
  // flags found" now reads as green/"Legitimate Website" just like a
  // recognized domain, instead of a separate neutral cyan state. The
  // trustLevel value itself still distinguishes verified vs unverified
  // internally (see History panel/badge text), so nothing about the actual
  // detection logic changed - only how the "no threats" case is styled.
  unverified: {
    heading: 'Legitimate Website',
    badgeBg: 'bg-emerald-500/20',
    badgeText: 'text-emerald-400',
...
export function ResultsPanel({ results, isAnalyzing }: ResultsPanelProps) {
  ...
  const verdict = verdictConfig[results.trustLevel];
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <span className={`inline-block px-2 py-0.5 ${
                ...
                {results.trustLevel === 'phishing' ? risk.label : 'SAFE'}
              </span>
