...
// FIX #5: real domain intelligence layer. The heuristic model above only
// pattern-matches URL structure - it has no way to know whether a domain
// that raises zero red flags is an established business or a brand-new
// throwaway site built to look clean. This adds two live, keyless data
// sources fetched through our own /api/domain-check proxy (to avoid CORS
// and keep any future API keys server-side):
//   - RDAP (rdap.org) for real domain registration age
//   - URLhaus (abuse.ch) for a known-malicious-host blocklist match
export interface DomainIntelligence {
  ...
  // Diagnostic-only: why the RDAP lookup came back empty, if it did.
  // Surfaced in the UI so a silent failure is debuggable instead of just
  // showing "Unavailable" with no explanation.
  rdapDebug?: string | null;
}
...
export interface URLAnalysisResult {
  ...
  // FIX #3 (three-tier verdict): 'phishing' = crossed the detection threshold;
  // 'verified_legitimate' = matches a domain we actually recognize as trusted;
  // 'unverified' = no phishing signals fired, but we also have no positive
  // evidence the domain is legitimate - distinct from a confirmed-safe brand.
  trustLevel: 'verified_legitimate' | 'unverified' | 'phishing';
  // Populated asynchronously after the initial heuristic result is shown -
  // starts undefined, then becomes { status: 'pending', ... } immediately,
  // then updates to 'success' or 'error' once the live lookup resolves.
  domainIntel?: DomainIntelligence;
}

// Shared hostname parser so the live domain-intelligence lookup checks
// exactly the same hostname the heuristic feature extractor used.
...
// Deterministic pseudo-random helper so the same URL always produces the same
// "simulated" value across scans, instead of a fresh Math.random() each time
// (which previously meant re-scanning the same URL could flip verdicts).
...
function extractFeatures(url: string): FeatureData[] {
  ...
  try {
    ...
    const ageSeed = hashString(hostname);
...
function calculatePhishProbability(features: FeatureData[]): { probability: number; shapValues: { feature: string; impact: number }[] } {
  ...
  let strongSignalCount = 0; // count of high/critical suspicious signals
  ...
  // FIX: compounding-risk bonus. In practice, two or more high/critical signals
  // appearing *together* on the same URL is far more damning than their sum
  // suggests in isolation (e.g. "no HTTPS" alone is common on harmless old
  // sites, but "no HTTPS" + "brand-new domain" together is a strong phishing
  // pattern). Reward that correlation instead of just adding the raw points.
  if (strongSignalCount >= 2) {
    phishScore += (strongSignalCount - 1) * 12;
  }

  // FIX #3b: combo bump for "no HTTPS" + "domain reputation unknown" together.
  // Neither is damning alone (plenty of harmless small/new sites lack both),
  // but the *combination* - no encryption AND nothing vouching for the domain -
  // is more suspicious than either signal in isolation, so it should nudge the
  // score up a notch rather than being silently absorbed by the safe default.
  const noHttps = features.find(f => f.name === 'SSL/HTTPS Certificate')?.isSuspicious;
  const unknownReputation = features.find(f => f.name === 'Domain Reputation')?.value === 'Unknown';
  if (noHttps && unknownReputation) {
    phishScore += 10;
  }

  // Normalize to 0-100 range.
  // FIX: lowered from 120 to 60 - the old divisor assumed far more signals
  // would typically fire together than realistically do, which let clearly
  // suspicious URLs (e.g. no HTTPS + brand-new domain) stay under the 50%
  // "phishing" threshold.
  let probability = Math.min(95, Math.round((phishScore / 60) * 100));
  ...
  // FIX: hard override for "smoking gun" features. These almost never appear
  // on a legitimate site under any circumstance, so their presence alone
  // should be enough to call it phishing regardless of how the rest of the
  // math shakes out.
  const CRITICAL_OVERRIDE_FEATURES = new Set([
    'IP Address Usage',
    '@ Symbol Presence',
    'Typosquatting',
    'Punycode/Homograph Domain',
  ]);
  const hasHardOverride = features.some(f => f.isSuspicious && CRITICAL_OVERRIDE_FEATURES.has(f.name));
  if (hasHardOverride) {
...
export function analyzeURL(url: string): URLAnalysisResult {
  ...
  // FIX #3c: trustLevel gives the UI a real third state instead of forcing
  // every non-phishing result into "Legitimate Website". Only a domain we
  // actually recognize (LEGITIMATE_DOMAINS, via the true match/subdomain
  // check) earns "verified_legitimate" - everything else that merely lacks
  // red flags is "unverified", since absence of evidence isn't evidence of
  // trustworthiness.
  const isRecognizedDomain = features.some(f => f.name === 'Domain Reputation' && f.value === 'Trusted');
  const trustLevel: URLAnalysisResult['trustLevel'] = isPhishing
    ? 'phishing'
    : isRecognizedDomain
    ? 'verified_legitimate'
    : 'unverified';
