...
// FIX #3d: verdict copy/iconography keyed off trustLevel rather than the
// binary isPhishing flag, so "no red flags found" (unverified) reads
// differently from "matches a domain we actually recognize" (verified).
// riskConfig above still drives the confidence-bar color/severity; this
// controls the headline icon, heading, and badge.
const verdictConfig = {
  phishing: {
    heading: 'PHISHING DETECTED!',
  ...
  blocklisted: {
    heading: 'Malicious Domain Detected',
    badgeBg: 'bg-red-500/20',
    badgeText: 'text-red-400',
    iconGradient: 'from-red-600 to-red-700',
    shadow: 'shadow-red-500/30',
...
export function ResultsPanel({ results, isAnalyzing }: ResultsPanelProps) {
  ...
  const intel = results.domainIntel;
  // FIX #5: layer live domain intelligence on top of the heuristic verdict.
  // A confirmed match on a real threat-intel feed (URLhaus) is treated as
  // definitive regardless of what the local heuristics found. Short of that,
  // a domain registered very recently (a classic throwaway-phishing-site
  // pattern) keeps an otherwise-clean "unverified" result from reading as
  // "Legitimate" - see FIX #4 just below for the clean-unverified case itself.
  ...
  // FIX #4: "unverified" previously meant "not on our small hardcoded brand
  // list" and was always rendered as red/Suspicious - which incorrectly
  // flagged perfectly clean, HTTPS-secured sites (e.g. a personal Vercel
  // deployment) purely because they're not Google/Amazon/etc. Now we only
  // show the red "Suspicious Website" treatment when the analyzer actually
  // found suspicious features (missing HTTPS, phishing keywords, etc.) or
  // live intelligence flags it as new. An unrecognized domain with zero red
  // flags and a normal registration age reads as Legitimate instead.
  const effectiveVerdictKey = isBlocklisted
    ? 'blocklisted'
    : results.trustLevel === 'unverified' && results.suspiciousFeatures.length === 0 && !isVeryNewDomain
    ? 'verified_legitimate'
    : results.trustLevel;
  const verdict = verdictConfig[effectiveVerdictKey];
      ...
      <div className="bg-slate-800/50 rounded-2xl border border-slate-700 p-6 backdrop-blur-sm">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${verdict.iconGradient} flex items-center justify-center shadow-lg ${verdict.shadow}`}>
            ...
            <div>
              <span className={`inline-block px-2 py-0.5 ${
                results.trustLevel === 'phishing' ? risk.bg : verdict.badgeBg
              } ${
                results.trustLevel === 'phishing' ? risk.text : verdict.badgeText
              } text-xs font-bold rounded-md mb-1`}>
                {results.trustLevel === 'phishing' ? risk.label : effectiveVerdictKey === 'verified_legitimate' ? 'LEGITIMATE' : effectiveVerdictKey === 'blocklisted' ? 'MALICIOUS' : 'SUSPICIOUS'}
              </span>
          ...
          <div className="text-right">
            <div className={`text-3xl font-black ${results.trustLevel === 'phishing' ? risk.text : verdict.badgeText}`}>
              {results.confidence}%
            </div>
        ...
        <div className="relative h-4 bg-slate-900 rounded-full overflow-hidden mb-4">
          <div
            className={`absolute top-0 left-0 h-full bg-gradient-to-r ${verdict.iconGradient} transition-all duration-1000 ease-out`}
      ...
      {intel && (
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700 p-6 backdrop-blur-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-semibold text-white">Live Domain Intelligence</h3>
              <p className="text-xs text-slate-400">Real-time registration age (RDAP) &amp; threat-feed check (URLhaus)</p>
            </div>
          </div>

          {intel.status === 'pending' && (
            <div className="flex items-center gap-2 text-sm text-slate-400">
              <svg className="w-4 h-4 animate-spin text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 12a8 8 0 018-8V2.5" />
              </svg>
              Checking domain registration age and threat feeds...
            </div>
          )}

          {intel.status === 'error' && (
            <div className="text-sm text-slate-400">
              Live lookup unavailable right now — the verdict above is based on the heuristic analysis only.
            </div>
          )}
          ...
          {intel.status === 'success' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="p-3 bg-slate-900/50 rounded-lg border border-slate-700">
                <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Domain Age</div>
                <div className={`text-sm font-semibold ${intel.domainAgeDays !== null && intel.domainAgeDays < 30 ? 'text-orange-400' : 'text-white'}`}>
                  {intel.domainAgeDays !== null ? `${intel.domainAgeDays} days` : 'Unavailable'}
                </div>
                {intel.domainAgeDays !== null && intel.domainAgeDays < 30 && (
                  <div className="text-[10px] text-orange-400 mt-1">Recently registered — a common phishing pattern</div>
                )}
                {intel.domainAgeDays === null && intel.rdapDebug && (
                  <div className="text-[10px] text-slate-500 mt-1 break-words">{intel.rdapDebug}</div>
                )}
              </div>
              <div className="p-3 bg-slate-900/50 rounded-lg border border-slate-700">
                <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Registrar</div>
                <div className="text-sm font-semibold text-white truncate">{intel.registrar ?? 'Unavailable'}</div>
              </div>
              <div className="p-3 bg-slate-900/50 rounded-lg border border-slate-700 sm:col-span-2">
                <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Threat Feed Match</div>
                <div className={`text-sm font-semibold ${intel.isBlocklisted ? 'text-red-400' : 'text-emerald-400'}`}>
                  {intel.isBlocklisted ? `Flagged by ${intel.blocklistSource}` : 'No match found'}
                </div>
              </div>
            </div>
          )}
